...
export default defineConfig(({ mode }) => {
    ...
    return {
        ...
        build: {
            ...
            minify: isProduction ? "terser" : "esbuild", // 开发环境使用更快的esbuild
            ...
            assetsInlineLimit: isProduction ? 8 * 1024 : 50 * 1024, // 生产环境降低内联阈值
            chunkSizeWarningLimit: 1000, // 规定触发警告的 chunk 大小, 这里设置阈值为50kb, 消除打包大小超过500kb警告
                ...
                output: {
                    ...
                    manualChunks: (id) => {
                        ...
                        if (id.includes('node_modules')) {
                            ...
                            if (id.includes('@visactor')) return 'charts';
                            // 其他库统一打包，减少分割复杂度
                            return 'vendor';
                        }
                    }
            ...
            reportCompressedSize: false, // 关闭压缩大小报告，减少构建输出
            // 开启构建缓存
            cache: true
        }
    };
