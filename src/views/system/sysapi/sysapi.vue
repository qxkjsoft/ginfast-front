      ...
      >
        <template #columns>
          ...
          <a-table-column title="操作" :width="200" align="center" :fixed="'right'">
            <template #cell="{ record }">
              <a-space>
                <a-button type="primary" size="mini" @click="onUpdate(record)" v-hasPerm="['system:api:edit']">
                  <template #icon><icon-edit /></template>
                  <span>修改</span>
                </a-button>
                <a-popconfirm type="warning" content="确定删除该项吗?" @ok="onDelete(record)">
                  <a-button type="primary" status="danger" size="mini" v-hasPerm="['system:api:delete']">
